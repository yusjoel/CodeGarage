        ...
        public ApkInfo extractInfo(XmlDocument manifestXml, byte[] resources_arsx)
        {
            ...
            try
            {
                ...
                if (!info.hasIcon && id != null)
                {
                    ...
                    string applicationLabelId = VER_ICN[LABEL_ID];
                    if (!applicationLabelId.Equals(""))
                    {
                        List<String> labels = null;
                        if (info.resStrings.ContainsKey(applicationLabelId))
                            labels = info.resStrings[applicationLabelId];
                        if (labels == null)
                        {
                            applicationLabelId = applicationLabelId.Replace("@string/", "");
                            if (info.resStrings.ContainsKey(applicationLabelId))
                                labels = info.resStrings[applicationLabelId];
                        }
